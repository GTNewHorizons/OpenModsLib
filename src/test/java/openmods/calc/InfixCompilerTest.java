...
public class InfixCompilerTest extends CalcTestUtils {
    ...
    public void testSimpleExpr() {
        // 1 + 2
        given(dec("1"), OP_PLUS, dec("2")).expect(c("1"), c("2"), PLUS);

        given(dec("1"), OP_MINUS, dec("2")).expect(c("1"), c("2"), MINUS);
    }

    @Test
    public void testMultipleSamePrecendenceOps() {
        // 1 + 2 - 3
        given(dec("1"), OP_PLUS, dec("2"), OP_MINUS, dec("3")).expect(c("1"), c("2"), PLUS, c("3"), MINUS);
    }

    @Test
    public void testMultipleDifferentPrecendenceOps() {
        // 1 + 2 * 3
        given(dec("1"), OP_PLUS, dec("2"), OP_MULTIPLY, dec("3")).expect(c("1"), c("2"), c("3"), MULTIPLY, PLUS);

        // 1 * 2 + 3
        given(dec("1"), OP_MULTIPLY, dec("2"), OP_PLUS, dec("3")).expect(c("1"), c("2"), MULTIPLY, c("3"), PLUS);
    }
    ...
    public void testBrackets() {
        ...
        // [1]
        given(leftBracket("["), dec("1"), rightBracket("]")).expect(c("1"));
    ...
    public void testNestedBrackets() {
        ...
        given(
                ...
                RIGHT_BRACKET,
                RIGHT_BRACKET,
                RIGHT_BRACKET).expect(c("1"), c("2"), c("3"), MINUS, PLUS);

        // ((1))
        given(LEFT_BRACKET, LEFT_BRACKET, dec("1"), RIGHT_BRACKET, RIGHT_BRACKET).expect(c("1"));
        ...
        given(
                ...
                OP_MINUS,
                dec("3"),
                RIGHT_BRACKET).expect(c("1"), c("2"), PLUS, c("3"), MINUS);
        ...
        given(
                ...
                OP_MINUS,
                dec("3"),
                rightBracket("]")).expect(c("1"), c("2"), PLUS, c("3"), MINUS);
    }
    ...
    public void testSymbolGet() {
        // pi
        given(symbol("pi")).expect(get("pi"));

        // pi + 2
        given(symbol("pi"), OP_PLUS, dec("2")).expect(get("pi"), c("2"), PLUS);

        // 2 + pi
        given(dec("2"), OP_PLUS, symbol("pi")).expect(c("2"), get("pi"), PLUS);

        // a + b
        given(symbol("a"), OP_PLUS, symbol("b")).expect(get("a"), get("b"), PLUS);

        // sin(a)
        given(symbol("sin"), LEFT_BRACKET, symbol("a"), RIGHT_BRACKET).expect(get("a"), call("sin", 1));
    }

    @Test
    public void testNullaryFunction() {
        // test()
        given(symbol("test"), LEFT_BRACKET, RIGHT_BRACKET).expect(call("test", 0));
    ...
    public void testUnaryFunction() {
        // sin(2)
        given(symbol("sin"), LEFT_BRACKET, dec("2"), RIGHT_BRACKET).expect(c("2"), call("sin", 1));
    ...
    public void testBinaryFunction() {
        ...
        given(
                ...
                OP_MINUS,
                dec("5"),
                RIGHT_BRACKET).expect(c("2"), c("3"), PLUS, c("4"), c("5"), MINUS, call("exp", 2));
    ...
    public void testFunctionSum() {
        ...
        given(
                ...
                LEFT_BRACKET,
                dec("3"),
                RIGHT_BRACKET).expect(c("2"), call("sin", 1), c("3"), call("cos", 1), PLUS);
        ...
        given(
                ...
                COMMA,
                dec("4"),
                RIGHT_BRACKET).expect(c("2"), call("sin", 1), c("3"), c("4"), call("exp", 2), PLUS);
    }
    ...
    public void testNestedFunctions() {
        ...
        given(
                ...
                dec("2"),
                RIGHT_BRACKET,
                RIGHT_BRACKET).expect(c("3"), call("sin", 1), c("4"), c("2"), call("cos", 1), PLUS, call("exp", 2));
    }

    @Test
    public void testUnaryOperators() {
        // -2
        given(OP_MINUS, dec("2")).expect(c("2"), UNARY_MINUS);

        // !3
        given(OP_NEG, dec("3")).expect(c("3"), UNARY_NEG);

        // 1 - -2
        given(dec("1"), OP_MINUS, OP_MINUS, dec("2")).expect(c("1"), c("2"), UNARY_MINUS, MINUS);

        // 1 * -2
        given(dec("1"), OP_MULTIPLY, OP_MINUS, dec("2")).expect(c("1"), c("2"), UNARY_MINUS, MULTIPLY);
        ...
        // (2) - 3
        given(LEFT_BRACKET, dec("2"), RIGHT_BRACKET, OP_MINUS, dec("3")).expect(c("2"), c("3"), MINUS);

        // -(2)
        given(OP_MINUS, LEFT_BRACKET, dec("2"), RIGHT_BRACKET).expect(c("2"), UNARY_MINUS);
        ...
        // -pi
        given(OP_MINUS, symbol("pi")).expect(get("pi"), UNARY_MINUS);
    ...
    public void testDoubleUnaryOperators() {
        // --2
        given(OP_MINUS, OP_MINUS, dec("2")).expect(c("2"), UNARY_MINUS, UNARY_MINUS);

        // -+2
        given(OP_MINUS, OP_PLUS, dec("2")).expect(c("2"), UNARY_PLUS, UNARY_MINUS);

        // -!2
        given(OP_MINUS, OP_NEG, dec("2")).expect(c("2"), UNARY_NEG, UNARY_MINUS);

        // !-+2
        given(OP_NEG, OP_MINUS, OP_PLUS, dec("2")).expect(c("2"), UNARY_PLUS, UNARY_MINUS, UNARY_NEG);
    }

    @Test
    public void testDefaultOperator() {
        // 2a == 2 * a
        given(dec("2"), symbol("a")).expect(c("2"), get("a"), DEFAULT).expectSameAs(dec("2"), OP_DEFAULT, symbol("a"));

        // -2a == -2 * a
        given(OP_MINUS, dec("2"), symbol("a")).expect(c("2"), UNARY_MINUS, get("a"), DEFAULT)
        ...
        // 2(a) = 2 * (a)
        given(dec("2"), LEFT_BRACKET, symbol("a"), RIGHT_BRACKET).expect(c("2"), get("a"), DEFAULT)
                .expectSameAs(dec("2"), OP_DEFAULT, symbol("a"));

        // 3 a + 2 = 3 * a + 2
        given(dec("3"), symbol("a"), OP_PLUS, dec("2")).expect(c("3"), get("a"), DEFAULT, c("2"), PLUS);

        // 3 + 2a == 3 + 2 * a
        given(dec("3"), OP_PLUS, dec("2"), symbol("a")).expect(c("3"), c("2"), get("a"), DEFAULT, PLUS)
        ...
        given(dec("2"), dec("3")).expect(c("2"), c("3"), DEFAULT).expectSameAs(dec("2"), OP_DEFAULT, dec("3"));

        // 3(2) == 3 * 2
        given(dec("3"), LEFT_BRACKET, dec("2"), RIGHT_BRACKET).expect(c("3"), c("2"), DEFAULT)
                .expectSameAs(dec("3"), OP_DEFAULT, dec("2"));

        // (2)3 = 2 * 3
        given(LEFT_BRACKET, dec("2"), RIGHT_BRACKET, dec("3")).expect(c("2"), c("3"), DEFAULT)
                .expectSameAs(dec("2"), OP_DEFAULT, dec("3"));

        // (2) i == 2 * i
        given(LEFT_BRACKET, dec("2"), RIGHT_BRACKET, symbol("i")).expect(c("2"), get("i"), DEFAULT)
        ...
        given(symbol("a"), LEFT_BRACKET, dec("2"), symbol("i"), COMMA, dec("4"), RIGHT_BRACKET)
                .expect(c("2"), get("i"), DEFAULT, c("4"), call("a", 2)).expectSameAs(
                        symbol("a"),
                        LEFT_BRACKET,
                        dec("2"),
                        OP_DEFAULT,
        ...
        given(LEFT_BRACKET, dec("2"), RIGHT_BRACKET, LEFT_BRACKET, dec("3"), RIGHT_BRACKET)
                .expect(c("2"), c("3"), DEFAULT).expectSameAs(dec("2"), OP_DEFAULT, dec("3"));
        ...
        // a[3] = a * [3]
        given(symbol("a"), leftBracket("["), dec("3"), rightBracket("]")).expect(get("a"), c("3"), DEFAULT)
                .expectSameAs(symbol("a"), OP_DEFAULT, dec("3"));

        // a 3 = a * 3
        given(symbol("a"), dec("3")).expect(get("a"), c("3"), DEFAULT).expectSameAs(symbol("a"), OP_DEFAULT, dec("3"));

        // a b = a * b
        given(symbol("a"), symbol("b")).expect(get("a"), get("b"), DEFAULT)
                .expectSameAs(symbol("a"), OP_DEFAULT, symbol("b"));

        // -a b = -a * b
        given(OP_MINUS, symbol("a"), symbol("b")).expect(get("a"), UNARY_MINUS, get("b"), DEFAULT)
    ...
    public void testValueSymbolQuote() {
        // quote(12)
        given(QUOTE_SYMBOL, LEFT_BRACKET, dec("12"), RIGHT_BRACKET)
                .expect(OPEN_ROOT_QUOTE_S, valueMarker("12"), CLOSE_ROOT_QUOTE_S);
        // quote("abc")
        given(QUOTE_SYMBOL, LEFT_BRACKET, string("abc"), RIGHT_BRACKET)
    ...
    public void testRawValueQuote() {
        // #abc
        given(QUOTE_MODIFIER, symbol("abc"))
                .expect(OPEN_ROOT_QUOTE_M, rawValueMarker(symbol("abc")), CLOSE_ROOT_QUOTE_M);
        // #+
