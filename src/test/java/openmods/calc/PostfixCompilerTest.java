...
public class PostfixCompilerTest extends CalcTestUtils {
    ...
    private CompilerResultTester given(Token... inputs) {
        return new CompilerResultTester(new DefaultPostfixCompiler<String>(VALUE_PARSER, operators) {
            ...
            protected IPostfixCompilerState<String> createStateForModifier(String modifier) {
                if (modifier.equals(TEST_MODIFIER)) return new TestModifierState();
                else return super.createStateForModifier(modifier);
            }
    ...
    public void testModifier() {
        ...
        given(dec("1"), mod(TEST_MODIFIER), symbol("a"), symbol("b"))
                .expect(c("1"), c(rawTokenString(symbol("a"))), call("b"));
    }
    ...
    public void testBrackets() {
        ...
        given(dec("1"), LEFT_BRACKET, dec("2"), RIGHT_BRACKET, dec("3"))
                .expect(c("1"), OPEN_BRACKET, c("2"), CLOSE_BRACKET, c("3"));
        given(dec("1"), LEFT_BRACKET, dec("2"), dec("3"), RIGHT_BRACKET, dec("4"))
                .expect(c("1"), OPEN_BRACKET, c("2"), c("3"), CLOSE_BRACKET, c("4"));
    }

    @Test
    public void testNestedBracketFlattening() {
        given(LEFT_BRACKET, LEFT_BRACKET, dec("4"), RIGHT_BRACKET, RIGHT_BRACKET)
                .expect(OPEN_BRACKET, OPEN_BRACKET, c("4"), CLOSE_BRACKET, CLOSE_BRACKET);
        given(LEFT_BRACKET, LEFT_BRACKET, LEFT_BRACKET, dec("4"), RIGHT_BRACKET, RIGHT_BRACKET, RIGHT_BRACKET)
                .expect(OPEN_BRACKET, OPEN_BRACKET, OPEN_BRACKET, c("4"), CLOSE_BRACKET, CLOSE_BRACKET, CLOSE_BRACKET);

        given(LEFT_BRACKET, dec("3"), LEFT_BRACKET, dec("4"), RIGHT_BRACKET, RIGHT_BRACKET)
                .expect(OPEN_BRACKET, c("3"), OPEN_BRACKET, c("4"), CLOSE_BRACKET, CLOSE_BRACKET);
    }
