...
public class TypedFunctionTest {
    ...
    private static List<TypedValue> execute(ICallable<TypedValue> f, OptionalInt argCount, int rets,
            TypedValue... values) {
        final Frame<TypedValue> frame = FrameFactory.createTopFrame();
        frame.stack().pushAll(Arrays.asList(values));
        f.call(frame, argCount, OptionalInt.of(rets));
        List<TypedValue> results = Lists.newArrayList();
        for (int i = 0; i < rets; i++) results.add(frame.stack().pop());
    ...
    public void testSingleMethodOptionalArgs() {
        ...
        Mockito.when(mock.test(anyBoolean(), Matchers.<Optional<String>>any(), Matchers.<Optional<Number>>any()))
                .thenReturn(7);
    ...
    public void testSingleMethodOptionalRawArgs() {
        ...
        final TypedValue arg2 = wrap("Hello world");
        final TypedValue arg3 = wrap(6);
        Mockito.when(mock.test(anyBoolean(), Matchers.<Optional<TypedValue>>any(), Matchers.<Optional<Number>>any()))
                .thenReturn(7);
    ...
    public void testSingleMethodOptionalAndVariadicArgs() {
        ...
        Mockito.when(mock.test(anyBoolean(), Matchers.<Optional<Number>>any(), Matchers.<Integer[]>anyVararg()))
                .thenReturn(5);
    ...
    public void testSingleMethodMultiIterableReturn() {
        ...
        Assert.assertEquals(
                execute(target, OptionalInt.absent(), 4),
                Arrays.asList(wrap("b"), wrap("c"), wrap("a"), wrap("d")));
        Mockito.verify(mock).test();
