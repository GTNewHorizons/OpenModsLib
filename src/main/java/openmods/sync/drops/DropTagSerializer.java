...
public class DropTagSerializer {
        ...
        while (cls != Object.class) {
            for (Field field : cls.getDeclaredFields()) {
                ...
                Preconditions.checkArgument(
                        ISyncableObject.class.isAssignableFrom(field.getType()),
                        "Field '%s' has SyncableDrop annotation, but isn't ISyncableObject",
                        field);
    ...
    public void write(NBTTagCompound tag) {
        for (Map.Entry<String, ISyncableObject> e : objects.entrySet()) e.getValue().writeToNBT(tag, e.getKey());
    }
