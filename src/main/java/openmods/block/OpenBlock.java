...
public abstract class OpenBlock extends Block implements IRegisterableBlock {
    ...
    public boolean hasCapabilities(TileEntityCapability... capabilities) {
        for (TileEntityCapability capability : capabilities) if (teCapabilities.contains(capability)) return true;
    ...
    public Orientation getInventoryRenderOrientation() {
        return inventoryRenderOrientation != null ? inventoryRenderOrientation
                : getRotationMode().getInventoryRenderOrientation();
    }

    @SideOnly(Side.CLIENT)
    public int getInventoryRenderMetadata(int itemMetadata) {
        final BlockRotationMode rotationMode = getRotationMode();
        final Orientation renderOrientation = inventoryRenderOrientation != null ? inventoryRenderOrientation
                : rotationMode.getInventoryRenderOrientation();
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        if (shouldDropFromTeAfterBreak()) {
            ...
            if (te != null) {
                ...
                for (ItemStack stack : getTileBreakDrops(te)) BlockUtils.dropItemStackInWorld(world, x, y, z, stack);
    ...
    @Override
    public void setupBlock(String modId, String blockName, Class<? extends TileEntity> tileEntity,
    ...
    public final static boolean areNeighborBlocksSolid(World world, int x, int y, int z, ForgeDirection... sides) {
        for (ForgeDirection side : sides) {
            if (isNeighborBlockSolid(world, x, y, z, side)) {
                return true;
            }
        }
        return false;
    ...
    public boolean canPlaceBlock(World world, EntityPlayer player, ItemStack stack, int x, int y, int z,
            ForgeDirection sideDir, Orientation blockOrientation, float hitX, float hitY, float hitZ, int newMeta) {
    ...
    /***
     * An extended block placement function which includes ALL the details you'll ever need. This is called if your
     * ItemBlock extends ItemOpenBlock
     */
    public void afterBlockPlaced(World world, EntityPlayer player, ItemStack stack, int x, int y, int z,
            ForgeDirection side, Orientation blockOrientation, float hitX, float hitY, float hitZ, int itemMeta) {
    ...
    protected void notifyTileEntity(World world, EntityPlayer player, ItemStack stack, int x, int y, int z,
            ForgeDirection side, Orientation blockOrientation, float hitX, float hitY, float hitZ) {
    ...
    public void setTextures(IIcon icon, ForgeDirection... directions) {
        for (ForgeDirection direction : directions) textures[direction.ordinal()] = icon;
    }
    ...
    /**
     * This method should be overriden if needed. We're getting the texture for the UNROTATED block for a particular
     * side (direction). Feel free to look up data in the TileEntity to grab additional information here
     */
    ...
    /***
     * This is called by the blockrenderer when rendering an item into the inventory. We'll return the block, rotated as
     * we wish, but without any additional texture changes that are caused by the blocks current state
     */
    @Override
    @SideOnly(Side.CLIENT)
