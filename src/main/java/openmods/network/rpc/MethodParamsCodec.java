...
public class MethodParamsCodec {
    ...
    MethodParamsCodec(Method method) {
        ...
        for (int i = 0; i < params.length; i++) this.params[i] = new MethodParam(types[i], annotations[i]);
    }
    ...
    public void writeArgs(DataOutput output, Object... args) {
        if (args == null) {
            Preconditions.checkArgument(
                    0 == params.length,
                    "Argument list length mismatch, expected %d, got 0",
                    params.length);
            return;
        }
        ...
        Preconditions.checkArgument(
                ...
                "Argument list length mismatch, expected %d, got %d",
                params.length,
                args.length);
    ...
    private static void writeArg(DataOutput output, int argIndex, IStreamWriter<Object> writer, boolean isNullable,
    ...
    private static Object readArg(DataInput input, IStreamReader<Object> reader, boolean isNullable)
            throws IOException {
        if (isNullable) {
