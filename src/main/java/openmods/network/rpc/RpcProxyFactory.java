...
public class RpcProxyFactory {
    ...
    public <T> T createProxy(ClassLoader loader, final IPacketSender sender, final IRpcTarget wrapper,
            Class<? extends T> mainIntf, Class<?>... extraIntf) {
        Class<?> allInterfaces[] = ArrayUtils.add(extraIntf, mainIntf);

        for (Class<?> intf : allInterfaces) Preconditions
                .checkState(registry.isClassRegistered(intf), "Class %s is not registered as RPC interface", intf);
