...
public class IdSyncManager extends DataStoreManager {
    ...
    private class InboundHandler extends SimpleChannelInboundHandler<FMLProxyPacket> {
        ...
        public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
            if (evt instanceof NetworkHandshakeEstablished) {
                Log.debug(
                        "Sending id data for player: %s",
                        OpenMods.proxy.getPlayerFromHandler(((NetworkHandshakeEstablished) evt).netHandler));
                sendAllIds(ctx);
            } else {
    ...
    public <K, V> DataStoreBuilder<K, V> createDataStore(String domain, String id, Class<? extends K> keyClass,
    ...
    public <K, V> DataStoreBuilder<K, V> createDataStore(String id, Class<? extends K> keyClass,
        ...
        Preconditions.checkState(
                ...
                "This method cannot be called in post-initialization state and later");
