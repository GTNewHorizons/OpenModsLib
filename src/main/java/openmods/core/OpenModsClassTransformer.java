...
public class OpenModsClassTransformer implements IClassTransformer {
        ...
        config.addEntry("activate_movement_callback", 0, "true", new ConfigOption("movement_callback") {
            ...
            protected void onActivate(final StateUpdater<TransformerState> state) {
                vanillaPatches.put(
                        ...
                        new TransformProvider(ClassWriter.COMPUTE_FRAMES) {
    ...
    public void injectAsmData(ASMDataTable table) {
        ...
        for (ASMData data : table.getAll("openmods.include.IncludeInterface")) includedClasses.add(data.getClassName());

        for (ASMData data : table.getAll("openmods.include.IncludeOverride")) includedClasses.add(data.getClassName());
    ...
    private boolean shouldTryIncluding(String clsName) {
        ...
        for (String prefix : IGNORED_PREFIXES) if (clsName.startsWith(prefix)) return false;
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        if (bytes == null) {
            return InjectedClassesManager.instance.tryGetBytecode(name);
        }
