...
public class DataStoreWrapper<K, V> {
    ...
    DataStoreWrapper(Map<K, V> localData, IStreamWriter<K> keyWriter, IStreamWriter<V> valueWriter,
            IStreamReader<K> keyReader, IStreamReader<V> valueReader) {
    ...
    private void notifyVisitors() {
        for (IDataVisitor<K, V> visitor : visitors) activeData.visit(visitor);
    }
