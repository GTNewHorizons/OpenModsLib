...
public class StructureRegistry {
    ...
    private static <T> void addMapGen(Collection<MapGenStructure> output, Class<T> klazz, T provider, String... names) {
        try {
            ...
            Log.warn(
                    e,
                    "Can't access fields %s from provider %s. Some structures may not be detected",
        ...
        builder.add(new IStructureGenProvider() {
            ...
            public Collection<MapGenStructure> listProviders(IChunkProvider provider) {
                ...
                try {
                    ...
                    Log.warn(
                            e,
                            "Can't access map gen list from provider %s. Some structures may not be detected",
                            provider);
                }
                return result;
            }
    ...
    private static IChunkProvider getWrappedChunkProvider(ChunkProviderServer provider) {
        try {
            return ReflectionHelper
                    .getPrivateValue(ChunkProviderServer.class, provider, "currentChunkProvider", "field_73246_d");
        } catch (UnableToAccessFieldException e) {
    ...
    private void visitStructures(WorldServer world, IStructureVisitor visitor) {
        ...
        if (inner != null) {
            for (IStructureGenProvider p : providers) if (p.canUseOnProvider(inner)) {
                for (MapGenStructure struct : p.listProviders(inner)) visitor.visit(struct);
            }
        }
    }
