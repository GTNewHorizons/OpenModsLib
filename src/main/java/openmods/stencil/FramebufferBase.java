...
public class FramebufferBase {
    ...
    protected void useExternalStencilBuffer(int buffer) {
        Preconditions.checkState(
                allocatedStencilBuffer == -1,
                "Trying to change stencil buffer, but one is already allocated");
    ...
    protected void useExternalDepthBuffer(int buffer) {
        Preconditions
                .checkState(allocatedDepthBuffer == -1, "Trying to change depth buffer, but one is already allocated");
    ...
    protected void allocateStencilBuffer(int format, int width, int height) {
        ...
        // OpenGlHelper.createRenderbufferStorage(OpenGlHelper.GL_RENDERBUFFER, FramebufferConstants.GL_STENCIL_FORMAT,
    ...
    protected boolean allocate() {
        ...
        bindFramebuffer();
        // OpenGlHelper.attachFramebufferTexture(OpenGlHelper.GL_FRAMEBUFFER, OpenGlHelper.GL_COLOR_ATTACHMENT0,
        ...
        if (usedDepthBuffer > -1) {
            // OpenGlHelper.attachRenderbuffer(OpenGlHelper.GL_FRAMEBUFFER, OpenGlHelper.GL_DEPTH_ATTACHMENT,
        ...
        if (usedStencilBuffer > -1) {
            // OpenGlHelper.attachRenderbuffer(OpenGlHelper.GL_FRAMEBUFFER, FramebufferConstants.GL_STENCIL_ATTACHMENT,
            // OpenGlHelper.GL_RENDERBUFFER, this.stencilBuffer);
            OpenGlHelper.func_153190_b(
                    OpenGlHelper.field_153198_e,
                    FramebufferConstants.GL_STENCIL_ATTACHMENT,
                    OpenGlHelper.field_153199_f,
                    usedStencilBuffer);
        }
