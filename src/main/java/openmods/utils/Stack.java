...
public class Stack<E> implements Iterable<E> {
    ...
    public E popAndExpectEmptyStack() {
        if (size() != 1) throw new StackUnderflowException(
                "Expected exactly one element, got %d, contents: %s",
                size(),
                printContents());
        return pop();
    }
    ...
    public Stack<E> substack(int depth) {
        ...
        if (newBottom < bottomElement) throw new StackUnderflowException(
                String.format("Not enough elements to create substack: required %s, size %d", depth, size()));
    ...
    public Stack<E> checkSizeIsExactly(int expectedSize) {
        if (size() != expectedSize) throw new StackUnderflowException(
                "Expected stack size %d, got %d, contents: %s",
                expectedSize,
                size(),
                printContents());
        return this;
