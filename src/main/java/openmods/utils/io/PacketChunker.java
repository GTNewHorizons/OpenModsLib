...
public class PacketChunker {
    ...
    /***
     * Get the bytes from the packet. If the total packet is not yet complete (and we're waiting for more to complete
     * the sequence), we return null. Otherwise we return the full byte array
     *
     * @param payload one of the chunks
    ...
    public synchronized byte[] consumeChunk(DataInput input, int payloadLength) throws IOException {
        ...
        for (byte[] s : alreadyReceived) if (s == null) return null; // not completed yet
