...
public class VanillaAnvilLogic {
    ...
    public VanillaAnvilLogic(ItemStack inputStack, ItemStack modifierStack) {
        ...
        if (modifierStack != null) {
            ...
            isEnchantedBook = modifierStack.getItem() == Items.enchanted_book
                    && Items.enchanted_book.func_92110_g(modifierStack).tagCount() > 0;

            if (inputStackCopy.isItemStackDamageable()
                    && inputStackCopy.getItem().getIsRepairable(inputStack, modifierStack)) {
                ...
                if (!isEnchantedBook && (inputStackCopy.getItem() != modifierStack.getItem()
                        || !inputStackCopy.isItemStackDamageable()))
                    return;
        ...
        for (Iterator<Integer> iterator1 = inputEnchantments.keySet().iterator(); iterator1
                .hasNext(); materialCost += k + k1 * l1) {
            i1 = iterator1.next();
            Enchantment enchantment = Enchantment.enchantmentsList[i1];
        ...
        if (isEnchantedBook && !inputStackCopy.getItem().isBookEnchantable(inputStackCopy, modifierStack))
            inputStackCopy = null;
