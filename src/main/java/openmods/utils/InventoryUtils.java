...
public class InventoryUtils {
    ...
    private static IInventory doubleChestFix(TileEntity te) {
        ...
        if (world.getBlock(x - 1, y, z) == block) return new InventoryLargeChest(
                "Large chest",
        ...
        if (world.getBlock(x + 1, y, z) == block) return new InventoryLargeChest(
                ...
                (IInventory) te,
                (IInventory) world.getTileEntity(x + 1, y, z));
        if (world.getBlock(x, y, z - 1) == block) return new InventoryLargeChest(
                "Large chest",
    ...
    public static Set<Integer> getAllSlots(IInventory inventory) {
        inventory = getInventory(inventory);
        Set<Integer> slots = Sets.newHashSet();
        for (int i = 0; i < inventory.getSizeInventory(); i++) slots.add(i);
    ...
    protected static void canInsert(ISidedInventory inventory, int slot, ForgeDirection side, ItemStack stack) {
        Preconditions.checkArgument(
                inventory.canInsertItem(slot, stack, side.ordinal()),
                "Item cannot be inserted into slot %s on side %s",
                slot,
                side);
    }
    ...
    protected static void canExtract(ISidedInventory inventory, int slot, ForgeDirection side, ItemStack stack) {
        Preconditions.checkArgument(
                inventory.canExtractItem(slot, stack, side.ordinal()),
                "Item cannot be extracted from slot %s on side %s",
                slot,
                side);
    }
