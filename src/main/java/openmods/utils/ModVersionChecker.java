 ...
 * version string. It uses NaturalOrderComparator.java to determine if the remote version is newer than our local
 * version.
 *
 * This is a non-nagging implementation. An example implementation of the source version of this version checker can be
 * found at: https://github.com/bspkrs/bspkrsCore/tree/master/testmod/bspkrs/testmod
 *
 * @author bspkrs
 */
...
public class ModVersionChecker {
    ...
    public ModVersionChecker(String modName, String oldVer, String versionURL, String updateURL, String[] loadMsg,
            String[] inGameMsg) {
        this(modName, oldVer, versionURL, updateURL, loadMsg, inGameMsg, 3000);
    }

    public ModVersionChecker(String modName, String currentVer, String versionURL, String updateURL, String[] loadMsg,
            String[] inGameMsg, int timeoutMS) {
    ...
    public ModVersionChecker(String modName, String oldVer, String versionURL, String updateURL) {
        this(
                ...
                new String[] { "\247c{modName} {newVer} \247ris out! Download the latest from \247a{updateURL}\247r" });
    }

    public void checkVersionWithLogging() {
        if (!isNewerVersion(currentVer, proposedVer)) for (String msg : loadMsg) Log.info(msg);
    }

    public void setLoadMessage(String[] loadMsg) {
        this.loadMsg = loadMsg;

        for (int i = 0; i < this.loadMsg.length; i++) this.loadMsg[i] = replaceAllTags(this.loadMsg[i]);
    }

    public void setInGameMessage(String[] inGameMsg) {
        this.inGameMsg = inGameMsg;

        for (int i = 0; i < this.inGameMsg.length; i++) this.inGameMsg[i] = replaceAllTags(this.inGameMsg[i]);
    ...
    private String replaceAllTags(String s) {
        return s.replace("{oldVer}", currentVer).replace("{newVer}", proposedVer).replace("{modName}", modName)
