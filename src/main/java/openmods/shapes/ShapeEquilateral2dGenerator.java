...
public class ShapeEquilateral2dGenerator extends DefaultShapeGenerator {
    ...
    @Override
    public void generateShape(int minX, final int minY, int minZ, int maxX, final int maxY, int maxZ,
            final IShapeable shapeable) {
        final IShapeable columnShapeable = new IShapeable() {

            @Override
            public void setBlock(int x, int ingored, int z) {
                for (int y = minY; y <= maxY; y++) shapeable.setBlock(x, y, z);
            }
        };
