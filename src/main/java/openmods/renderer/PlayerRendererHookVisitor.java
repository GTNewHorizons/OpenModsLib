...
public class PlayerRendererHookVisitor extends ClassVisitor {
        ...
        public InjectorMethodVisitor(MethodVisitor mv) {
            ...
            try {
                postMethod = Method.getMethod(
                        PlayerRendererHookVisitor.class.getMethod("post", AbstractClientPlayer.class, float.class));
            } catch (Throwable t) {
                throw Throwables.propagate(t);
            }

            Log.debug(
                    "Injecting hook %s.%s into EntityPlayerRender.rotateCorpse",
                    PlayerRendererHookVisitor.class,
                    postMethod);
        }
        ...
        public void visitInsn(int opcode) {
            if (opcode == Opcodes.RETURN) {
                ...
                visitMethodInsn(
                        ...
                        hookCls.getInternalName(),
                        postMethod.getName(),
                        postMethod.getDescriptor(),
                        false);
                listener.onSuccess();
            }
    ...
    public PlayerRendererHookVisitor(String rendererTypeCls, ClassVisitor cv, IResultListener listener) {
        ...
        Type injectedMethodType = Type.getMethodType(
                Type.VOID_TYPE,
                MappedType.of(AbstractClientPlayer.class).type(),
                Type.FLOAT_TYPE,
                Type.FLOAT_TYPE,
                Type.FLOAT_TYPE);
        modifiedMethod = new MethodMatcher(
                rendererTypeCls,
                injectedMethodType.getDescriptor(),
                "rotateCorpse",
                "func_77043_a");
    }
