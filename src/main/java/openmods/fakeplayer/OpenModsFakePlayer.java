...
public class OpenModsFakePlayer extends FakePlayer {
    ...
    public boolean tryPlaceItem(ItemStack itemStack, int x, int y, int z, ForgeDirection side, float hitX, float hitY,
            float hitZ) {
        if (itemStack == null) return false;

        final int opposite = side.getOpposite().ordinal();
        PlayerInteractEvent event = ForgeEventFactory
                .onPlayerInteract(this, Action.RIGHT_CLICK_BLOCK, x, y, z, opposite, worldObj);
        ...
        if (usedItem.onItemUseFirst(itemStack, this, worldObj, x, y, z, opposite, hitX, hitY, hitZ)) {
            return true;
        }
        ...
        try {
            ...
            Log.warn(
                    t,
                    "Invalid use of fake player with item %s @ (%d,%d,%d), aborting. Don't do it again",
                    usedItem,
                    x,
                    y,
                    z);
