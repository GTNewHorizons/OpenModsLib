...
public class ApiProviderRegistry<A> {
    ...
    private <T extends A> void registerInterfaces(Class<? extends T> cls, ApiInstanceProvider<T> provider,
    ...
    public <T extends A> void registerClass(Class<? extends T> cls) {
        ...
        ApiInstanceProvider<T> provider = isCacheable(meta) ? new ApiInstanceProvider.CachedInstance<T>(cls)
                : new ApiInstanceProvider.NewInstance<T>(cls);
