...
public class GuiComponentSideSelector extends BaseComponent implements IValueReceiver<Set<ForgeDirection>> {
    ...
    public GuiComponentSideSelector(int x, int y, double scale, Block block, int meta, TileEntity te,
            boolean highlightSelectedSides) {
        super(x, y);
        this.scale = scale;
    ...
    public void setValue(IReadableBitMap<ForgeDirection> dirs) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) if (dirs.get(dir)) selectedSides.add(dir);
    }
