...
public abstract class FixedCallable<E> implements ICallable<E> {
    ...
    public final void call(Frame<E> frame, OptionalInt argumentsCount, OptionalInt returnsCount) {
        if (!argumentsCount.compareIfPresent(argCount)) throw new StackValidationException(
                "Expected %s argument(s) but got %s",
                this.argCount,
                argumentsCount.get());
        if (!returnsCount.compareIfPresent(resultCount)) throw new StackValidationException(
