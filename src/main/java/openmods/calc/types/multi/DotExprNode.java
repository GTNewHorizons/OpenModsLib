...
public abstract class DotExprNode extends BinaryOpNode<TypedValue> {
    ...
    public DotExprNode(IExprNode<TypedValue> left, IExprNode<TypedValue> right, BinaryOperator<TypedValue> operator,
            TypeDomain domain) {
    ...
    public static class Basic extends DotExprNode {
        ...
        public Basic(UnaryOperator<TypedValue> unpackMarker, IExprNode<TypedValue> left, IExprNode<TypedValue> right,
                BinaryOperator<TypedValue> operator, TypeDomain domain) {
        ...
        private void appendSymbolApply(final List<IExecutable<TypedValue>> output,
                Iterable<IExprNode<TypedValue>> children) {
    ...
    public static class NullAware extends DotExprNode {
        ...
        public NullAware(IExprNode<TypedValue> left, IExprNode<TypedValue> right, BinaryOperator<TypedValue> operator,
                TypeDomain domain) {
            super(left, right, operator, domain);
        }
        ...
        private void flattenKeyNode(List<IExecutable<TypedValue>> output) {
            if (right instanceof SymbolCallNode) { // disabled, for symmetry with [...], use .?member?(), which is more
                                                   // correct anyway
