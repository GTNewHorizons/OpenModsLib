...
public class OptionalType {
    ...
    private static TypedValue createPresentConstructor(final TypeDomain domain) {
        return domain.create(
                TypeUserdata.class,
                new TypeUserdata("optional.present", Value.class),
                MetaObject.builder().set(MetaObjectUtils.callableAdapter(new UnaryFunction.Direct<TypedValue>() {
                    ...
                    public Optional<List<TypedValue>> tryDecompose(TypedValue self, TypedValue input, int variableCount,
                        ...
                        Preconditions.checkArgument(
                                variableCount == 1,
                                "Invalid number of values to unpack, expected none got %s",
                                variableCount);
                ...
                }).set(TypeUserdata.defaultStrSlot).set(TypeUserdata.defaultReprSlot)
                        .set(TypeUserdata.defaultAttrSlot(domain)).build());
    }
    ...
    private static TypedValue createAbsentConstructor(final TypeDomain domain) {
        return domain.create(
                TypeUserdata.class,
                new TypeUserdata("optional.absent", Value.class),
                MetaObject.builder().set(MetaObjectUtils.callableAdapter(new NullaryFunction.Direct<TypedValue>() {
    ...
    private static TypedValue createOptionalType(final TypedValue nullValue, final TypedValue presentTypeValue,
            final TypedValue absentTypeValue) {
        ...
        final Map<String, TypedValue> methods = ImmutableMap.<String, TypedValue>builder()
                ...
                })).put("present", presentTypeValue).put("absent", absentTypeValue)
                .put("name", domain.create(String.class, "optional")).build();

        return domain.create(
                TypeUserdata.class,
                new TypeUserdata("optional", Value.class),
                MetaObject.builder().set(TypeUserdata.defaultStrSlot).set(TypeUserdata.defaultReprSlot)
                        .set(MetaObjectUtils.DECOMPOSE_ON_TYPE).set(MetaObjectUtils.attrFromMap(methods))
                        .set(MetaObjectUtils.dirFromIterable(methods.keySet())).build());
    }
