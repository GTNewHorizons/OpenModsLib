...
public class ClosureCompilerHelper {
    ...
    public void compile(List<IExecutable<TypedValue>> output, Iterable<IExprNode<TypedValue>> args,
            IExprNode<TypedValue> lambdaBody) {
        final Optional<String> varArgName = compileArgs(output, args);
        if (varArgName.isPresent()) compileClosureVarCall(output, lambdaBody, varArgName.get());
        else compileClosureCall(output, lambdaBody);
    }
    ...
    private Optional<String> compileArgs(List<IExecutable<TypedValue>> output, Iterable<IExprNode<TypedValue>> args) {
        ...
        for (IExprNode<TypedValue> arg : args) {
            ...
            if (!newVarArgName.isPresent()) {
                ...
                if (varArgName.isPresent()) throw new IllegalStateException(
                        "Duplicate vararg: " + varArgName.get() + " - > " + newVarArgName.get());
    ...
    private void compileClosureVarCall(List<IExecutable<TypedValue>> output, IExprNode<TypedValue> lambdaBody,
            String varArgName) {
