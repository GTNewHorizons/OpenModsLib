...
public class TypedBinaryOperator extends BinaryOperator.Direct<TypedValue> {
    ...
    public static class Builder {
        ...
        private Builder registerCoercedOperation(Class<?> type, IGenericOperation op) {
            ...
            Preconditions
                    .checkState(prev == null, "Duplicate operation registration on operator '%s', type: %s", id, type);
            return this;
        }
        ...
        private static <T> IGenericOperation createOperationWrapper(final Class<T> type,
                final ICoercedOperation<? super T> op) {
        ...
        public <T, O> Builder registerOperation(Class<T> type, Class<O> output,
                ISimpleCoercedOperation<? super T, ? extends O> op) {
            return registerCoercedOperation(type, createOperationWrapper(type, output, op));
        }

        private static <T, O> IGenericOperation createOperationWrapper(final Class<T> type, final Class<O> output,
                final ISimpleCoercedOperation<? super T, ? extends O> op) {
            return new IGenericOperation() {
        ...
        private Builder registerVariantOperation(Class<?> left, Class<?> right, IGenericOperation op) {
            ...
            Preconditions.checkState(
                    prev == null,
                    "Duplicate operation registration on operator '%s', types: %s, %s",
                    id,
                    left,
                    right);
            return this;
        ...
        public <L, R> Builder registerOperation(Class<? extends L> left, Class<? extends R> right,
                IVariantOperation<? super L, ? super R> op) {
            return registerVariantOperation(left, right, createOperationWrapper(left, right, op));
        }

        private static <L, R> IGenericOperation createOperationWrapper(final Class<? extends L> left,
                final Class<? extends R> right, final IVariantOperation<? super L, ? super R> op) {
            return new IGenericOperation() {
        ...
        public <L, R, O> Builder registerOperation(Class<? extends L> left, Class<? extends R> right,
                Class<? super O> output, ISimpleVariantOperation<? super L, ? super R, ? extends O> op) {
            return registerVariantOperation(left, right, createOperationWrapper(left, right, output, op));
        }

        private static <O, R, L> IGenericOperation createOperationWrapper(final Class<? extends L> left,
                final Class<? extends R> right, final Class<? super O> output,
                final ISimpleVariantOperation<? super L, ? super R, ? extends O> op) {
            return new IGenericOperation() {
        ...
        public TypedBinaryOperator build(TypeDomain domain) {
            for (IGenericOperation op : coercedOperations.values()) op.validate(domain);

            for (IGenericOperation op : variantOperations.values()) op.validate(domain);
    ...
    private TypedBinaryOperator(String id, int precedence, Associativity associativity, TypeDomain domain,
            Map<Class<?>, IGenericOperation> coercedOperations,
            Table<Class<?>, Class<?>, IGenericOperation> variantOperations, IDefaultOperation defaultOperation) {
    ...
    public TypedValue execute(TypedValue left, TypedValue right) {
        ...
        throw new IllegalArgumentException(
                String.format("Can't apply operation '%s' on values %s,%s", id, left, right));
    }
}
