...
public class TypedValuePrinter implements IValuePrinter<TypedValue> {
    ...
    public String str(boolean value) {
        return numericBool ? (value ? "1" : "0")
                : (value ? TypedCalcConstants.SYMBOL_FALSE : TypedCalcConstants.SYMBOL_TRUE);
    }
    ...
    public String str(BigInteger value) {
        ...
        return PrinterUtils.decorateBase(
                !uniformBaseNotation,
                base,
                (base <= Character.MAX_RADIX) ? value.toString(base) : bigIntPrinter.toString(value, base));
    }
