...
public class MatchExpressionFactory {
    ...
    public MatchExpressionFactory(TypeDomain domain, BinaryOperator<TypedValue> split,
            BinaryOperator<TypedValue> lambda) {
        this.domain = domain;
        this.split = split;
        this.lambda = lambda;
    ...
    private abstract static class PatternMatcher {
        ...
        public Optional<Code> match(Frame<TypedValue> env, SymbolMap<TypedValue> output, List<TypedValue> values) {
            Preconditions.checkState(
                    values.size() == argPatterns.size(),
                    "Invalid usage: expected %s values, got %s",
                    argPatterns.size(),
                    values.size());
    ...
    private static class GuardedPatternMatcher extends PatternMatcher {
        ...
        public GuardedPatternMatcher(List<IBindPattern> patterns, List<GuardedAction> guardedActions,
                Optional<Code> defaultAction) {
            super(patterns);
    ...
    private class PatternCompiler {
        ...
        public void flatten(List<IExecutable<TypedValue>> output) {
            ...
            for (ActionCompiler action : actions) action.flatten(patternCompileCode);
        ...
        public PatternCompiler apply(IExprNode<TypedValue> input) {
            ...
            if (patternNode.operator == lambda) {
                ...
                return new PatternCompiler(varMatchers, compilers);
                    ...
                    "Invalid 'match' syntax, expected '->' between pattern and action or \\ between pattern and guarded actions, got"
                            + patternNode.operator);
        }
        ...
        private void extractGuards(List<ActionCompiler> compilers, IExprNode<TypedValue> clause) {
            if (clause instanceof BinaryOpNode) {
                ...
                if (op.operator == split) {
                    ...
                    Preconditions.checkState(
                            op.left instanceof BinaryOpNode,
                            "Invalid 'match' syntax: expected guard -> action on left side of \\, got: ",
                            op.left);
                    final BinaryOpNode<TypedValue> left = (BinaryOpNode<TypedValue>) op.left;
                    Preconditions.checkState(
                            left.operator == lambda,
                            "Invalid 'match' syntax: expected guard -> action on left side of \\, got: %s",
                            left.operator);
