...
public class LibListFunctions {
    ...
    private static TypedValue executeUnaryCallable(Frame<TypedValue> frame, TypedValue callable,
            MetaObject.SlotCall slotCall, TypedValue arg) {
    ...
    public static void register(Environment<TypedValue> env) {
        ...
        env.setGlobalSymbol("filter", new BinaryFunction.WithFrame<TypedValue>() {
            ...
            @Override
            protected TypedValue call(final Frame<TypedValue> frame, final TypedValue predicate,
                    final TypedValue list) {
        ...
        env.setGlobalSymbol("reduce", new TernaryFunction.WithFrame<TypedValue>() {
            ...
            @Override
            protected TypedValue call(final Frame<TypedValue> frame, final TypedValue functor,
                    final TypedValue initialValue, final TypedValue list) {
                final MetaObject.SlotCall slotCall = functor.getMetaObject().slotCall;
                final Stack<TypedValue> stack = frame.stack();
        ...
        env.setGlobalSymbol(TypedCalcConstants.SYMBOL_FLATTEN, new SingleReturnCallable<TypedValue>() {
            ...
            public TypedValue call(Frame<TypedValue> frame, OptionalInt argumentsCount) {
                ...
                for (TypedValue value : values) Iterables.addAll(result, Cons.toIterable(value, nullValue));
        ...
        env.setGlobalSymbol("sort", new SingleReturnCallable<TypedValue>() {
            ...
            public TypedValue call(final Frame<TypedValue> frame, OptionalInt argumentsCount) {
                ...
                if (reverse != null && MetaObjectUtils.boolValue(frame, reverse)) elements = Lists.reverse(elements);

                return Cons.createList(elements, nullValue);
            }

            private Comparator<TypedValue> extractCompareFunction(final Frame<TypedValue> frame,
                    final Stack<TypedValue> stack, final TypedValue compareFunctionArg) {
                if (compareFunctionArg != null) {
                    final MetaObject.SlotCall slotCall = compareFunctionArg.getMetaObject().slotCall;
                    Preconditions.checkState(
                            slotCall != null,
                            "'Compare function' argument %s is not callable",
                            compareFunctionArg);
            ...
            private KeyFunction extractKeyFunction(final Frame<TypedValue> frame, final Stack<TypedValue> stack,
                    final TypedValue keyFunctionArg) {
                if (keyFunctionArg != null) {
                    final MetaObject.SlotCall slotCall = keyFunctionArg.getMetaObject().slotCall;
                    Preconditions
                            .checkState(slotCall != null, "'Key function' argument %s is not callable", keyFunctionArg);
        ...
        env.setGlobalSymbol("reverse", new UnaryFunction.Direct<TypedValue>() {
            ...
            protected TypedValue call(TypedValue value) {
                ...
                for (TypedValue v : Cons.toIterable(value, nullValue)) result = Cons.create(domain, v, result);
