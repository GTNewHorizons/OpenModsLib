...
public class BindPatternEvaluator {
    ...
    private abstract static class PatternMatchConstructor implements IBindPattern {
        ...
        public boolean match(Frame<TypedValue> env, SymbolMap<TypedValue> output, TypedValue value) {
            ...
            if (decomposer != null) {
                ...
                final Optional<List<TypedValue>> maybeDecomposition = decomposer
                        .tryDecompose(typeValue, value, expectedValueCount, env);
                ...
                Preconditions.checkState(
                        actualValueCount == expectedValueCount,
                        "Decomposable contract broken - returned different number of values: expected: %s, got %s",
                        expectedValueCount,
                        actualValueCount);
        ...
        public void listBoundVars(Collection<String> output) {
            for (IBindPattern pattern : argMatchers) pattern.listBoundVars(output);
        }
        ...
        protected String serializeArgNames() {
            ...
            for (IBindPattern arg : argMatchers) args.add(arg.serialize());
    ...
    private static class NamespaceCtorPlaceholder implements IBindPatternProvider {
        ...
        public IBindPattern getPattern(BindPatternTranslator translator) {
            throw new IllegalStateException(
                    "Unfinished namespace constructor matcher: " + var + "." + Joiner.on(".").join(path));
        }
    ...
    private static List<IBindPattern> translatePatterns(BindPatternTranslator translator, List<TypedValue> args) {
        ...
        for (TypedValue m : args) varMatchers.add(translator.translatePattern(m));
    ...
    private final MetaObject namespaceCtorPlaceholderMetaObject = MetaObject.builder().set(new MetaObject.SlotAttr() {
        ...
        public Optional<TypedValue> attr(TypedValue self, String key, Frame<TypedValue> frame) {
            ...
            return Optional
                    .of(domain.create(IBindPatternProvider.class, placeholder.extend(key), self.getMetaObject()));
        }
        ...
        public void call(TypedValue self, OptionalInt argumentsCount, OptionalInt returnsCount,
            ...
            Preconditions
                    .checkArgument(argumentsCount.isPresent(), "Type constructor must be always called with arg count");
    ...
    public TypedValue evaluate(SymbolMap<TypedValue> topSymbolMap, Code pattern) {
        final Frame<TypedValue> patternFrame = FrameFactory
                .symbolsToFrame(new PatternPlaceholdersSymbolMap(topSymbolMap));
