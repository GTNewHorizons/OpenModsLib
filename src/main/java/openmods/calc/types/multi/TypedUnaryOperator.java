    ...
    public static class Builder {
        ...
        private Builder registerOperation(Class<?> argCls, final IGenericOperation op) {
            ...
            Preconditions.checkState(
                    prev == null,
                    "Duplicate operation registration on operator '%s' for type %s",
                    id,
                    argCls);
            return this;
        }
        ...
        private static <A> IGenericOperation createOperationWrapper(final Class<? extends A> argCls,
                final IOperation<? super A> op) {
        ...
        public <A, R> Builder registerOperation(Class<? extends A> argCls, Class<? super R> resultCls,
                ISimpleOperation<? super A, ? extends R> op) {
            return registerOperation(argCls, createOperationWrapper(argCls, resultCls, op));
        }

        private static <A, R> IGenericOperation createOperationWrapper(final Class<? extends A> argCls,
                final Class<? super R> resultCls, final ISimpleOperation<? super A, ? extends R> op) {
            return new IGenericOperation() {
        ...
        public TypedUnaryOperator build(TypeDomain domain) {
            for (IGenericOperation op : operations.values()) op.validate(domain);

            return new TypedUnaryOperator(id, operations, defaultOperation, domain);
        }
    }

    public TypedUnaryOperator(String id, Map<Class<?>, IGenericOperation> operations,
            IDefaultOperation defaultOperation, TypeDomain domain) {
        super(id);
