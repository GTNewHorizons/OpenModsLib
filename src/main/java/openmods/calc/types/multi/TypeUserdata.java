...
class TypeUserdata {
    ...
    protected Optional<TypedValue> attr(TypeDomain domain, String key) {
        ...
        if (ATTR_TYPE_METAOBJECT.equals(key))
            return Optional.of(domain.create(MetaObject.class, domain.getDefaultMetaObject(type)));
    ...
    public static MetaObject.Builder defaultMetaObject(TypeDomain domain) {
        return MetaObject.builder().set(defaultStrSlot).set(defaultReprSlot).set(MetaObjectUtils.USE_VALUE_EQUALS)
                .set(MetaObjectUtils.DECOMPOSE_ON_TYPE).set(defaultAttrSlot(domain)).set(defaultDirSlot());
    }
}
