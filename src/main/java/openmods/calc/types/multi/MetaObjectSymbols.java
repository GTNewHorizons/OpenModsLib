...
public class MetaObjectSymbols {
    ...
    private static MetaObject.Builder createBuilderFromArgs(Iterable<TypedValue> args) {
        ...
        for (TypedValue arg : args) {
            if (arg.is(MetaObjectSlot.class)) {
                ...
                final MetaObjectInfo.SlotAccess slotInfo = pair.car
                        .as(MetaObjectInfo.SlotAccess.class, "slot:value pair");
                ...
                if (slotValue.is(MetaObjectSlot.class)) {
                    ...
                    Preconditions.checkState(
                            nativeSlot.info == slotInfo,
                            "Invalid slot type for name %s, got %s",
                            slotInfo.name,
                            nativeSlot.info.name);
    ...
    public static void register(Environment<TypedValue> env) {
        ...
        {
            final TypedValue metaObjectSlotType = domain
                    .create(TypeUserdata.class, new TypeUserdata("metaobjectslotvalue", MetaObjectSlot.class));
            env.setGlobalSymbol("metaobjectslotvalue", metaObjectSlotType);
            domain.registerType(
                    MetaObjectSlot.class,
                    "metaobjectslotvalue",
                    createMetaObjectSlotMetaObject(metaObjectSlotType));
        }

        {
            final TypedValue metaObjectType = domain.create(
                    TypeUserdata.class,
                    new TypeUserdata("metaobject", MetaObject.class),
                    TypeUserdata.defaultMetaObject(domain).set(new MetaObject.SlotCall() {
                        ...
                        public void call(TypedValue self, OptionalInt argumentsCount, OptionalInt returnsCount,
                            ...
                            Preconditions.checkState(
                                    ...
                                    "'metaobject' symbol requires arguments count");
            ...
            domain.registerType(
                    MetaObject.class,
                    "metaobject",
                    createMetaObjectMetaObject(domain, metaObjectType, nullValue));
        }
    ...
    private static MetaObject createMetaObjectSlotMetaObject(TypedValue metaObjectSlotType) {
        return MetaObject.builder().set(new MetaObject.SlotCall() {
            ...
            public Optional<TypedValue> attr(TypedValue self, String key, Frame<TypedValue> frame) {
                ...
                if (key.equals(ATTR_INFO))
                    return Optional.of(domain.create(MetaObjectInfo.SlotAccess.class, slot.info));
                return Optional.absent();
            }
        }).set(MetaObjectUtils.dirFromArray(ATTR_NAME, ATTR_INFO)).set(MetaObjectUtils.typeConst(metaObjectSlotType))
                .build();
    }

    // much meta
    private static MetaObject createMetaObjectMetaObject(final TypeDomain domain, TypedValue metaObjectType,
            final TypedValue nullValue) {
