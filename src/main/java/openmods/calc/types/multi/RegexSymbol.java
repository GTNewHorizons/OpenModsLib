...
public class RegexSymbol {
    ...
    private static void addFlag(Map<String, TypedValue> output, TypeDomain domain, int value, String... names) {
        for (String name : names) output.put(name, wrap(domain, value));
    }
    ...
    private static MetaObject createPatternWrapperMetaObject(final TypeDomain domain, TypedValue patternType) {
        return MetaObject.builder().set(new MetaObject.SlotAttr() {
            ...
            public Optional<TypedValue> attr(TypedValue self, String key, Frame<TypedValue> frame) {
                ...
                if (key.equals(ATTR_PATTERN))
                    return Optional.of(domain.create(String.class, patternWrapper.pattern.pattern()));
                if (key.equals(ATTR_FLAGS)) return Optional.of(wrap(domain, patternWrapper.pattern.flags()));
                if (key.equals(ATTR_SEARCH))
                    return Optional.of(domain.create(MatcherWrapper.class, patternWrapper.search()));
                if (key.equals(ATTR_MATCH))
                    return Optional.of(domain.create(MatcherWrapper.class, patternWrapper.match()));

                return Optional.absent();
            }
        }).set(MetaObjectUtils.dirFromArray(ATTR_PATTERN, ATTR_FLAGS, ATTR_SEARCH, ATTR_MATCH))
                .set(MetaObjectUtils.typeConst(patternType)).set(new MetaObject.SlotStr() {
        ...
        return MetaObject.builder().set(new MetaObject.SlotCall() {
            ...
            public Optional<List<TypedValue>> tryDecompose(TypedValue self, TypedValue input, int variableCount,
                ...
                Preconditions.checkArgument(
                        variableCount == 1,
                        "Invalid number of variables, expected one, got %s",
                        variableCount);
                final MatcherWrapper matcher = self.as(MatcherWrapper.class);
    ...
    private static MetaObject createMatchWrapperMetaObject(final TypeDomain domain, final TypedValue nullValue,
            TypedValue matchType) {
        return MetaObject.builder().set(new MetaObject.SlotAttr() {
    ...
    public static void register(Environment<TypedValue> env) {
...

        {
            ...
            domain.registerType(
                    PatternWrapper.class,
                    "regex.pattern",
                    createPatternWrapperMetaObject(domain, patternType));
        }

        {
            ...
            domain.registerType(
                    ...
                    "regex.matcher",
                    createMatcherWrapperMetaObject(domain, matcherType));
        }

        {
            ...
            domain.registerType(
                    MatchWrapper.class,
                    "regex.match",
                    createMatchWrapperMetaObject(domain, nullValue, matchType));
        }

        final TypedValue regex = domain.create(
