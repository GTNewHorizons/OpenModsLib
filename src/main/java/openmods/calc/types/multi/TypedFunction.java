...
public class TypedFunction {
    ...
    public static class Builder {
        ...
        private static final Ordering<TypeVariant> VARIANT_ORDERING = Ordering.natural().reverse()
                .onResultOf(new Function<TypeVariant, Integer>() {
        ...
        public Builder addVariant(Method method) {
            if (!Modifier.isStatic(method.getModifiers())) allowedClasses.add(method.getDeclaringClass());
        ...
        public Builder addVariants(Class<?> cls) {
            for (Method m : cls.getMethods()) if (m.isAnnotationPresent(Variant.class)) addVariant(m);
        ...
        public IUnboundCallable build(Class<?> targetCls) {
            ...
            if (targetCls == null) {
                ...
                for (Class<?> cls : this.allowedClasses)
                    if (!cls.isAssignableFrom(targetCls)) throw new NonCompatibleMethodsPresent(cls, targetCls);
            }
        ...
        public ICallable<TypedValue> build(TypeDomain domain, Object target) {
            ...
            if (target == null) {
                ...
                for (Class<?> cls : this.allowedClasses)
                    if (!cls.isInstance(target)) throw new NonCompatibleMethodsPresent(cls, target);
            }
        ...
        private static TypedFunctionBody createMultiFunction(final List<TypeVariant> variants,
                final OptionalInt mandatoryArgNum) {
    ...
    private abstract static class TypeVariant {
        ...
        public TypeVariant(Method method, Map<Integer, DispatchArgMatcher> dispatchArgMatchers,
                List<ArgConverter> argConverters, int mandatoryArgNum) {
            this.method = method;
            ...
            this.lastDispatchArg = dispatchArgMatchers.isEmpty() ? -1
                    : Ordering.natural().max(dispatchArgMatchers.keySet());
        }
        ...
        private boolean isMatchAmbigous(TypeVariant other) {
            ...
            for (int i = 0; i < Math.max(thisLength, otherLength); i++) {
                final DispatchArgMatcher ownMatcher = i < thisLength ? this.dispatchArgMatchers.get(i)
                        : DispatchArgMatcher.MISSING;
                final DispatchArgMatcher otherMatcher = i < otherLength ? other.dispatchArgMatchers.get(i)
                        : DispatchArgMatcher.MISSING;
        ...
        private boolean matchDispatchArgs(List<TypedValue> args) {
            ...
            for (Map.Entry<Integer, DispatchArgMatcher> m : dispatchArgMatchers.entrySet()) {
                ...
                final Class<?> matchedArgType = matchedArgIndex < argCount ? args.get(matchedArgIndex).type
                        : MissingType.class;
        ...
        private List<Object> convertArgs(TypeDomain domain, List<TypedValue> args) {
            ...
            for (TypedValue v : args) Preconditions.checkArgument(v.domain == domain, "Mixed domain on arg %s", v);

            final Iterator<TypedValue> argsIterator = args.iterator();
            for (ArgConverter converter : argConverters) results.add(converter.convert(argsIterator));
    ...
    private static TypeVariant createVariant(final Method method) {
        ...
        for (int i = 0; i < parameterCount; i++) {
            ...
            Preconditions
                    .checkArgument(!(isTypedDispatch && isRawNonDispatch), "Argument cannot be both dispatch and raw");
            Preconditions.checkArgument(
                    !(isRawDispatch && isRawNonDispatch),
                    "Argument cannot be both raw and raw dispatch");
            Preconditions.checkArgument(
                    !(isRawDispatch && isTypedDispatch),
                    "Argument cannot be both raw and typed dispatch");
            Preconditions
                    .checkArgument(!(isVariadicArg && isDispatchArg), "Variadic arguments cannot be used for dispatch");

            if (isVariadicArg) {
                final Class<?> componentType = type.getComponentType().getRawType();
                if (isRawArg) {
                    Preconditions.checkState(
                            TypedValue.class.isAssignableFrom(componentType),
                            "Raw argument must have TypedValue type");
                ...
                Preconditions.checkState(
                        Optional.class.isAssignableFrom(type.getRawType()),
                        "Optional argument must have Optional type");
                final Class<?> varType = OptionalTypeHolder.resolve(type);
                if (isRawArg) {
                    Preconditions.checkState(
                            TypedValue.class.isAssignableFrom(varType),
                            "Raw argument must have TypedValue type");
                ...
                if (isRawArg) {
                    Preconditions.checkState(
                            TypedValue.class.isAssignableFrom(rawType),
                            "Raw argument must have TypedValue type");
        ...
        if (MultipleReturn.class.isAssignableFrom(returnType)) {
            ...
            Preconditions.checkState(
                    !isCollectionReturn,
                    "Method returning MultipleReturn cannot be marked as @MultiReturn");
            ...
            if (returnType.isArray()) {
                ...
                throw new IllegalArgumentException(
                        "Method " + method + " is marked with @MultiReturn, but does not return array or Iterable");
            }
        } else if (isRawReturn) {
            Preconditions.checkState(
                    TypedValue.class.isAssignableFrom(method.getReturnType()),
                    "Method marked with @RawReturn must return TypedValue");
    ...
    private abstract static class TypedFunctionBody {
        ...
        public void call(TypeDomain domain, Object target, Frame<TypedValue> frame, OptionalInt argumentsCount,
                OptionalInt returnsCount) {
            final int argCount;
            ...
            if (argumentsCount.isPresent()) {
                ...
                Preconditions.checkState(
                        mandatoryArgNum.isPresent(),
                        "Number of arguments not given and function is not fixed");
                argCount = mandatoryArgNum.get();
            }

            final List<TypedValue> reversedArgs = Lists.newArrayList();
            final Stack<TypedValue> stack = frame.stack();
            for (int i = 0; i < argCount; i++) reversedArgs.add(stack.pop());
            ...
            if (returnsCount.isPresent()) {
                ...
                Preconditions.checkState(
                        expectedReturns == actualReturns,
                        "Invalid number of return values, requested %s, got %s",
                        expectedReturns,
                        actualReturns);
            }
