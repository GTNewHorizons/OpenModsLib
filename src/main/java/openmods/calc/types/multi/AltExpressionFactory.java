...
public class AltExpressionFactory {
    ...
    public AltExpressionFactory(TypeDomain domain, TypedValue nullValue, BinaryOperator<TypedValue> colonOperator,
            BinaryOperator<TypedValue> assignOperator, BinaryOperator<TypedValue> splitOperator) {
        this.domain = domain;
    ...
    private class AltConstructorCompiler {
        ...
        public void flatten(List<IExecutable<TypedValue>> output) {
            ...
            for (String s : parts) result.add(domain.create(Symbol.class, Symbol.get(s)));
    ...
    private class AltNode extends ScopeModifierNode {
        ...
        @Override
        protected void flattenNameAndValue(List<IExecutable<TypedValue>> output, IExprNode<TypedValue> nameNode,
                IExprNode<TypedValue> ctorsNode) {
            ...
            for (AltConstructorCompiler ctor : ctors) ctor.flatten(output);

            output.add(new SymbolCall<TypedValue>(TypedCalcConstants.SYMBOL_LIST, ctors.size(), 1));
        }

        private void flattenConstructorDefinitionList(List<AltConstructorCompiler> output,
                IExprNode<TypedValue> ctorsNode) {
            if (ctorsNode instanceof BinaryOpNode) {
                final BinaryOpNode<TypedValue> opNode = (BinaryOpNode<TypedValue>) ctorsNode;
                Preconditions.checkState(
                        opNode.operator == splitOperator,
                        "Malformed constructor list, expected '\\', got %s",
                        opNode.operator);
    ...
    private class AltTypeVariant extends TypeUserdata {
        ...
        public AltTypeVariant(String name, AltType type, List<String> members) {
            ...
            final List<TypedValue> convertedMembers = ImmutableList
                    .copyOf(Iterables.transform(members, domain.createWrappingTransformer(String.class)));
        ...
        return MetaObject.builder().set(new MetaObject.SlotDecompose() {
            ...
            public Optional<List<TypedValue>> tryDecompose(TypedValue self, TypedValue input, int variableCount,
                ...
                Preconditions.checkArgument(
                        variableCount == variant.members.size(),
                        "Invalid number of values to unpack, expected %s got %s",
                        variant.members.size(),
                        variableCount);
                ...
                if (input.is(AltValue.class)) {
                    ...
                    if (value.variant == variant) {
                        ...
                        Preconditions.checkState(
                                values.size() == variant.members.size(),
                                "Mismatched size in container: names: %s, values: %s",
                                variant.members,
                                values);
            ...
            @Override
            public Optional<TypedValue> attr(TypedValue self, String key, Frame<TypedValue> frame) {
                if (TypeUserdata.ATTR_TYPE_NAME.equals(key))
                    return Optional.of(domain.create(String.class, self.as(AltTypeVariant.class).name));
    ...
    private static MetaObject createAltValueMetaObject() {
        return MetaObject.builder().set(new MetaObject.SlotStr() {
            ...
            public String str(TypedValue self, Frame<TypedValue> frame) {
                ...
                return "<alt " + value.variant.type.id
                        + ":"
                        + value.variant.id
                        + "("
    ...
    private class AltSymbol implements ICallable<TypedValue> {
        ...
        private void visitTypeDefinition(SymbolMap<TypedValue> symbols, TypedValue typeDefinition) {
            ...
            for (Map.Entry<String, TypedValue> e : newType.members.entrySet()) symbols.put(e.getKey(), e.getValue());
        ...
        private void visitConstructor(Map<String, TypedValue> variants, AltType type, TypedValue ctorDefinition) {
            ...
            if (membersList.is(Cons.class)) {
                ...
                Preconditions.checkState(
                        membersList == nullValue,
                        "Expected list or null (empty list), got %s",
                        membersList);
            }

            variants.put(
                    name.value,
                    domain.create(AltTypeVariant.class, new AltTypeVariant(name.value, type, memberNames)));
        }
