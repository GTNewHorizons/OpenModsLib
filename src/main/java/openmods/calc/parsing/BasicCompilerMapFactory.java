...
public class BasicCompilerMapFactory<E> implements ICompilerMapFactory<E, ExprType> {
    ...
    private static class WrappedCompiler<E> implements ICompiler<E> {
        ...
        public IExecutable<E> compile(String input) {
            ...
            if (tokens.hasNext()) throw new IllegalStateException("Unconsumed tokens: " + Lists.newArrayList(tokens));
    ...
    public Compilers<E, ExprType> create(E nullValue, IValueParser<E> valueParser, OperatorDictionary<E> operators,
        ...
        compilers
                .put(ExprType.PREFIX, new WrappedCompiler<E>(prefixTokenizer, createPrefixParser(prefixCompilerState)));
        compilers.put(ExprType.INFIX, new WrappedCompiler<E>(infixTokenizer, createInfixParser(infixCompilerState)));
        compilers.put(
                ExprType.POSTFIX,
                new WrappedCompiler<E>(postfixTokenizer, createPostfixParser(valueParser, operators, environment)));
    ...
    protected MappedCompilerState<E> createPrefixCompilerState(OperatorDictionary<E> operators,
            IExprNodeFactory<E> exprNodeFactory) {
    ...
    protected MappedCompilerState<E> createInfixCompilerState(OperatorDictionary<E> operators,
            IExprNodeFactory<E> exprNodeFactory) {
    ...
    protected ITokenStreamCompiler<E> createPostfixParser(IValueParser<E> valueParser, OperatorDictionary<E> operators,
    ...
    public static <E> DefaultPostfixCompiler<E> addConstantEvaluatorState(IValueParser<E> valueParser,
            OperatorDictionary<E> operators, Environment<E> env, DefaultPostfixCompiler<E> compiler) {
        return compiler.addBracketStateProvider(
                BRACKET_CONSTANT_EVALUATE,
                createConstantEvaluatorStateProvider(valueParser, operators, env, BRACKET_CONSTANT_EVALUATE));
    }

    public static <E> IStateProvider<E> createConstantEvaluatorStateProvider(final IValueParser<E> valueParser,
            final OperatorDictionary<E> operators, final Environment<E> env, final String openingBracket) {
        return new IStateProvider<E>() {

            @Override
            public IPostfixCompilerState<E> createState() {
                final IExecutableListBuilder<E> listBuilder = new DefaultExecutableListBuilder<E>(
                        valueParser,
                        operators);
                ...
                class ConstantEvaluatorState extends BracketPostfixCompilerStateBase<E> {
                    ...
                    protected IExecutable<E> processCompiledBracket(final IExecutable<E> compiledExpr) {
                        ...
                        for (E value : resultFrame.stack()) computedValues.add(Value.create(value));
