...
public class StringEscaper {
    ...
    public static final BiMap<Character, Character> ESCAPES = ImmutableBiMap.<Character, Character>builder()
            .put('\\', '\\').put('\'', '\'').put('"', '"').put('0', '\0').put('b', '\b').put('t', '\t').put('n', '\n')
            ...
            if (nonEscapeChars.contains(ch)) {
                ...
                if (possibleEscape != null) {
                    ...
                    Preconditions.checkState(
                            pos < value.length(),
                            "Malformed UTF-16 string: high surrogate at end of string");
                    final char nextCh = value.charAt(pos++);
                    Preconditions.checkState(
                            Character.isLowSurrogate(nextCh),
                            "Malformed UTF-16 string: expected low surrogate, got: %s",
                            Integer.toHexString(nextCh));
