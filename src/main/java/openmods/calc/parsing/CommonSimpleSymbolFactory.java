...
public class CommonSimpleSymbolFactory<E> {
    ...
    private class FailStateTransition extends SingleStateTransition.ForSymbol<E> {
        ...
        public IExprNode<E> createRootNode(List<IExprNode<E>> children) {
            Preconditions.checkState(
                    children.size() <= 1,
                    "'fail' expects at most single argument, got %s",
                    children.size());
            if (children.isEmpty()) {
    ...
    public class ExtendedCompilerMapFactory extends BasicCompilerMapFactory<E> {
        ...
        protected void configureCompilerStateCommon(MappedCompilerState<E> compilerState, Environment<E> environment) {
            ...
            compilerState.addStateTransition(
                    SYMBOL_CONSTANT,
                    new ConstantSymbolStateTransition<E>(compilerState, environment, SYMBOL_CONSTANT));
        }
    }
    ...
    private static <E> ISymbolBinder<E> createLetFunction(final String name, SymbolCallNode<E> symbolCallNode,
            IExprNode<E> valueNode) {
            ...
            public void bind(SymbolMap<E> localSymbols, final Frame<E> enclosingFrame) {
                class LetFunction implements ICallable<E> {
                    ...
                    public void call(Frame<E> callSiteFrame, OptionalInt argumentsCount, OptionalInt returnsCount) {
                        ...
                        if (!argumentsCount.compareIfPresent(expectedArgCount)) throw new StackValidationException(
                                "Expected %s argument(s) but got %s",
                                expectedArgCount,
                                argumentsCount.get());

                        final Frame<E> executionFrame = FrameFactory
                                .newLocalFrameWithSubstack(enclosingFrame, expectedArgCount);
                        final Stack<E> argStack = executionFrame.stack();
                        for (String arg : reversedArgs) executionFrame.symbols().put(arg, argStack.pop());

                        exprExecutable.execute(executionFrame);

                        final int actualReturns = argStack.size();
                        if (!returnsCount.compareIfPresent(actualReturns)) throw new StackValidationException(
                                "Has %s result(s) but expected %s",
                                actualReturns,
                                returnsCount.get());
                    }
    ...
    private class LetExecutable implements IExecutable<E> {
        ...
        public void execute(Frame<E> frame) {
            ...
            for (ISymbolBinder<E> e : variables) e.bind(letFrame.symbols(), frame);
    ...
    private class LetNode implements IExprNode<E> {
        ...
        public void flatten(List<IExecutable<E>> output) {
            ...
            Preconditions.checkState(
                    argsNode instanceof SquareBracketContainerNode,
                    "Malformed 'let' expressions: expected brackets, got %s",
                    argsNode);
        ...
        private ImmutableList<ISymbolBinder<E>> collectVars(final SquareBracketContainerNode<E> bracketNode) {
            ...
            for (IExprNode<E> argNode : bracketNode.getChildren()) flattenArgNode(varsBuilder, argNode);
            return varsBuilder.build();
        }

        private void flattenArgNode(ImmutableList.Builder<ISymbolBinder<E>> output, IExprNode<E> argNode) {
            Preconditions.checkState(
                    argNode instanceof BinaryOpNode,
                    "Expected expression in from <name>:<expr>, got %s",
                    argNode);
            final BinaryOpNode<E> opNode = (BinaryOpNode<E>) argNode;
            Preconditions.checkState(
                    keyValueSeparators.contains(opNode.operator),
                    "Expected operators %s as separator, got %s",
                    keyValueSeparatorsIds,
                    opNode.operator.id);
    ...
    public void registerSeparators(OperatorDictionary<E> operators) {
        for (BinaryOperator<E> separator : keyValueSeparators) operators.registerBinaryOperator(separator);
    }
