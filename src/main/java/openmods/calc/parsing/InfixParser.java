...
public class InfixParser<E> implements IAstParser<E> {
        ...
        while (input.hasNext()) {
            ...
            if (token.type.isValue()) {
                ...
                Preconditions.checkArgument(
                        token.type != TokenType.SYMBOL_WITH_ARGS,
                        "Symbol '%s' can't be used in infix mode",
                        token.value);
                ...
                if (input.hasNext()) {
                    ...
                    if (nextToken.type == TokenType.LEFT_BRACKET && nextToken.value.equals(CALL_OPENING_BRACKET)) {
                        ...
                        final List<IExprNode<E>> childrenNodes = collectChildren(
                                input,
                                openingBracket,
                                closingBracket,
    ...
    private List<IExprNode<E>> collectChildren(PeekingIterator<Token> input, String openingBracket,
            String closingBracket, ICompilerState<E> compilerState) {
...

        {
            ...
            if (input.peek().type == TokenType.RIGHT_BRACKET) {
                ...
                Preconditions.checkState(
                        token.value.equals(closingBracket),
